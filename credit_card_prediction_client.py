...
for i in range(0,len(reports)-1):
    ...
    requestPrediction  = model_pb2.Features(reports = reports[i], expenditure = expenditure[i],
                                        age = age[i], income = income[i])
...
print('That means you can do {} predictions in one second'.format(int(1/(all_time/len(reports)))))
